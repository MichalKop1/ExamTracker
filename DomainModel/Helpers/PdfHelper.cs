...
public class PdfHelper
{
    ...
    public void CreatePdfInvoice(string filePath, Invoice invoice, List<ProductService> productServiceList)
    {
        using (var writer = new PdfWriter(filePath))
        {
            using (var pdf = new PdfDocument(writer))
            {
                ...
                var invoiceLogo = System.Drawing.Image.FromFile("Assets\\invoice_logo.png");
                ImageData logoImage = ImageDataFactory.Create(ToByteArray(invoiceLogo));
                ...
                Paragraph invNum = new Paragraph(new Text($"{InvoiceNumber} {invoice.InvoiceNumber}").SetFontSize(12).SetFont(boldStandard)).SetTextAlignment(TextAlignment.RIGHT);
