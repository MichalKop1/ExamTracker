...
public class SQLiteMaturaExamRepository:IMaturaExamRepository
{
    ...
    public async Task<List<ExamMatura>> GetAllExams(int id)
    {
        ...
        try
        {
            ...
            {
                return (await connection.QueryAsync<ExamMatura>(query)).ToList();
            }
        }
        catch (Exception ex)
        {
    ...
    {
        string query = $"DELETE FROM MaturaExams WHERE exam_id = '{id}'";
        try
        {
            using (IDbConnection connection = new SQLiteConnection(ConnectionHelper.SQLiteConnectionString))
            {
