...
public class Grade8ExamRepository : IGrade8ExamRepository
{
    ...
    public async Task<List<Exam8Grade>> GetAllExams(int id)
    {
        ...
        try
        {
            ...
            {
                return (await connection.QueryAsync<Exam8Grade>(query, id)).ToList();
            }
        }
        catch (Exception ex)
        {
    ...
    {
        string query = $"DELETE FROM Grade8Exams WHERE exam_id = @ExamId";
        try
        {
            using (IDbConnection connection = new SqlConnection(ConnectionHelper.ConnectionString))
            {
